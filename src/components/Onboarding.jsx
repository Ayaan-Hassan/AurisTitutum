...
const Onboarding = ({ onAddHabit, habits = [], userConfig: propUserConfig, updateUserConfig: propUpdateUserConfig }) => {
  ...
  // Auto-trigger for brand new users: no onboardingComplete flag AND no habits
  // Also trigger via sessionStorage (for explicit triggers from landing page)
  const sessionTrigger = sessionStorage.getItem("triggerOnboarding") === "true";
  useEffect(() => {
    if (sessionTrigger) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      <Card className="w-full max-w-lg p-8 space-y-8 bg-bg-main border border-border-color shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] relative overflow-hidden">
        ...
        {step === 1 ? (
          <div className="relative z-10 space-y-7 animate-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="space-y-2 relative">
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="w-full h-[46px] rounded-xl border border-border-color bg-bg-main/50 backdrop-blur-md px-4 text-left text-xs text-text-primary outline-none focus:border-accent transition-all cursor-pointer hover:border-accent flex items-center justify-between"
                    >
                        ...
                        <div className="absolute top-full left-0 right-0 mt-2 bg-bg-sidebar border border-border-color rounded-xl overflow-hidden shadow-2xl z-[100] animate-in fade-in zoom-in-95 duration-200">
                          {[
                            ...
                            <button
                              key={opt.value}
                              type="button"
                              onClick={() => { setProfile(prev => ({ ...prev, gender: opt.value })); setIsGenderOpen(false); }}
