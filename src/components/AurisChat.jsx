...
export default function AurisChat({ user, isOpen, onClose, userConfig, habits, notes, reminders, notifications }) {
  ...
  | Recent History: ${logsStr}`;
    ...
    const notesContext = notes && notes.length > 0 
      ? `Notes Analysis:\n${notes.slice(-20).map(n => `- ${n.isLocked ? '[SECURE NODE]' : n.title}: ${n.isLocked ? '[ACCESS RESTRICTED: THIS NOTE IS FULLY ENCRYPTED AND LOCKED BY USER]' : (n.body?.substring(0, 150) + (n.body?.length > 150 ? '...' : ''))} ${n.metadata ? `[Behavioral Meta: ${n.metadata}]` : ''}`).join('\n')}`
